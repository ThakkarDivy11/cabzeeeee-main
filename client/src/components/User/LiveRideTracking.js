    ...
    const LiveRideTracking = () => {
        ...
        useEffect(() => {
            ...
            socketService.onStatusUpdate((data) => {
                ...
                // Sync the ride state with the data from socket
                setRide(prev => {
                    const base = prev || {};
                    const rideUpdate = data.ride || {};
                    return { ...base, ...rideUpdate, status: data.status };
                });

                // Update local display status
                if (data.status === 'accepted') {
                    setStatus('Driver Coming');
                    toast.success('Driver is on the way!', { icon: '🚗' });
                    ...
                    toast.error('This ride has been cancelled.', { icon: '🚫' });
                    setTimeout(() => navigate('/rider'), 3000);
                }
            });
        ...
        const handleCancelRide = async () => {
            const confirmed = window.confirm('Are you sure you want to cancel your ride?');
            if (!confirmed) return;

            try {
                const token = localStorage.getItem('token');
                const response = await fetch(`${process.env.REACT_APP_API_URL || 'http://localhost:5000'}/api/rides/${rideId}/status`, {
                    method: 'PUT',
                    headers: {
                ...
                <aside className="sidebar-dashboard">
                    ...
                    {['pending', 'accepted'].includes(ride.status) && (
                        <div className="mt-8 px-2">
                            ...
                            className="w-full py-4 rounded-2xl border border-red-500/30 bg-red-500/5 text-red-500 text-[10px] font-black uppercase tracking-[0.2em] hover:bg-red-500 hover:text-white transition-all duration-300"
                            >
                                Cancel Ride
                            </button>
                            <p className="text-[9px] text-white/20 text-center mt-3 font-medium uppercase tracking-widest">
